...
def _parse_timestamp(value: str | None) -> dt.datetime:
    ...
    try:
        ...
        logger.debug("Unable to parse Pinnacle timestamp %s", value)
        return dt.datetime.now(dt.timezone.utc)
...
class PinnacleScraper(SportsbookScraper):
    """Concrete scraper for Pinnacle JSON feeds."""
    ...
    async def _fetch_lines_impl(self) -> List[OddsQuote]:
        ...
        for event in events:
            ...
            markets = event.get("markets", []) or event.get("periods", [])
            for market in markets:
                group = market.get("group") or market.get("groupName") or market.get("category") or "Other"
                market_key = (
                    market.get("key")
                ...
                scope = market.get("scope") or market.get("period") or market.get("betScope") or "game"
                entity_type = market.get("entityType") or market.get("entity") or "team"
                outcomes: Iterable[Dict[str, Any]] = market.get("outcomes", []) or market.get("prices", [])
                for outcome in outcomes:
                    ...
                    try:
                        ...
                        logger.debug("Skipping Pinnacle outcome with price %s", price_raw)
                        continue
                    line = _safe_float(
                        outcome.get("line")
                        or outcome.get("points")
                        or outcome.get("handicap")
                        or outcome.get("spread")
                    )
                    side = outcome.get("side") or outcome.get("designation") or outcome.get("choice")
                    extra: Dict[str, Any] = {}
                    if market_id := market.get("id"):
                        extra["market_id"] = market_id
                    if outcome_id := outcome.get("id"):
                        extra["outcome_id"] = outcome_id
                    if alt := outcome.get("alt"):
                        extra["alt"] = alt
        ...
        logger.debug("Pinnacle scraper produced %d quotes", len(quotes))
        return quotes
